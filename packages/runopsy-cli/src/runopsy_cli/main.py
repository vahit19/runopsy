...
def _report_injections(store: Path | None = None) -> None:
    """Score the engine against faults injected into a clean run.
    ...
    With ``--store`` the healthy run is one that was **actually recorded** rather than
    generated, which is the closest this project can get to a labelled real corpus
    without a human. It is not a substitute for one — a person reading a genuine failure
    and saying where it began is still the measurement that decides whether this is
    useful — but the ground truth here needs no expert and is not an opinion: the onset
    is the step the fault was inserted at, which is a fact about the construction rather
    than a judgement about the trace. Real timings, real commands, real repository
    state; only the break is ours.
    """
    from runopsy_bench import score_injections
    from runopsy_bench.cases import all_cases

    if store is not None:
        events, source = _healthiest_recorded_run(store)
        if events is None:
            errors.print(
                f"No clean recorded run in {store} to break.\n"
                "  Injection needs a run that succeeded: record one, then try again.",
                style="yellow",
            )
            raise typer.Exit(code=2)
        console.print(f"Breaking a real recorded run: {source}\n", style="dim")
        scores = score_injections(events)
    else:
    ...
    from runopsy_bench.injection import INJECTORS

    # Kinds that produced no scoreable case at all have no row, so without this they
    # vanish from the table entirely and it reads as a clean sweep. The two that
    # disappear are the hardest ones — a plan cut off mid-sentence and a value that is
    # simply wrong — and a score that quietly omits what the engine cannot do is the
    # kind of narrowing this project treats as a defect everywhere else.
    absent = sorted(kind.value for kind in INJECTORS if kind not in {s.kind for s in scores})
    skipped = sum(score.skipped_undetectable for score in scores)

    if skipped or absent:
        parts = []
        if skipped:
            parts.append(f"{skipped} injected fault(s)")
        if absent:
            parts.append(f"every case of {', '.join(absent)}")
        console.print(
            f"\nExcluded: {' and '.join(parts)} — they leave nothing anomalous in the "
            "trace, so no deterministic detector can reach them. That is the boundary of "
            "this table, not a gap in it: those need the semantic layer or a replay.",
            style="dim",
        )


def _healthiest_recorded_run(store: Path | None) -> tuple[list[Event] | None, str]:
    """The longest recorded run that the engine finds nothing wrong with.

    Clean is the requirement, not merely successful. Injecting a fault into a run that
    already contains one would make the ground truth ambiguous — two things are now
    wrong and the label names only one — and the score would be measuring the engine's
    choice between them rather than whether it found the fault.

    Longest, because a longer run puts more distance between the injected onset and the
    symptom, which is the distance this engine exists to close.
    """
    best: tuple[list[Event], str] | None = None
    with Collector.open(store) as collector:
        for summary in collector.runs():
            events = list(collector.events(summary.run_id))
            if len(events) < 4:
                continue
            context = AnalysisContext.from_events(summary.run_id, events)
            if run_diagnosis(context).candidates:
                continue  # not clean; breaking it would give an ambiguous answer
