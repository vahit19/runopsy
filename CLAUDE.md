...
4. **No `llm_call` events were captured at all.** Hermes did not fire `post_llm_call` during a normal session, though the hook is configured and our handler maps its payload correctly when fired by hand (`hermes hooks test post_llm_call`). So real Hermes traces carry no token counts, cost or model latency, and the budget detector is blind on them. This one is upstream, not ours.

Both corrections left the benchmark at 94.4% top-1 and zero false positives. That is the finding, not a footnote: real traces did not improve the score, they changed what the score was failing to measure. Known and unfixed — a loop's onset is reported at the repeated call's *first* occurrence, which for the stuck run is step 1, well before the agent was actually stuck.

Three sessions are not a corpus, and all three used one model on small tasks. Every headline number still comes from constructed traces, so prefer work that records more real runs over work that adds surface.
