...
4. **No `llm_call` events were captured at all**, and the reason turned out to be ours to fix. Hermes 0.19.0 dispatches `post_llm_call` and `on_session_finalize` only through the **plugin** path (`hermes_cli.plugins.invoke_hook`); its shell-hook dispatcher emits exactly five events, listed in the table in `agent/shell_hooks.py`. We were telling users to configure two hooks that can never fire. `hermes hooks test post_llm_call` reports success because it calls the shell dispatcher directly with a synthetic payload, which is why the mistake survived so long — the adapter now registers only the four observational events that exist, keeps the other two mapped in `PLUGIN_ONLY_EVENTS` in case a later version routes them, and says plainly that model calls are not captured. Token counts, cost and model latency are therefore unavailable through shell hooks at all, and the budget detector is blind on real Hermes traces. Closing that needs a Hermes plugin — and even then the current `post_llm_call` payload carries no usage figures.
