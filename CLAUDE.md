...
To record more: install `hermes-agent` in its own venv, point `~/AppData/Local/hermes/config.yaml` (or the path from `hermes config path`) at the hooks that `runopsy adapter hermes` prints — using an **unquoted** absolute path, since `command: "path" hook x` is invalid YAML and Hermes silently falls back to its default config — then run with `RUNOPSY_HOME` set and `--accept-hooks`.
